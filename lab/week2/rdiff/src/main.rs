    ...
    for i in 0..m {
        for j in 0..n {
            if seq1.get(i) == seq2.get(j) {
                let val = lcs_table.get(i, j).unwrap();
                lcs_table.set(i + 1, j + 1, val + 1).unwrap();
            }
            else {
                let val = cmp::max(lcs_table.get(i + 1, j).unwrap(), lcs_table.get(i, j + 1).unwrap());
...
fn main() {
    ...
    let seq2 = read_file_lines(filename2).expect("Open file1 failed");
